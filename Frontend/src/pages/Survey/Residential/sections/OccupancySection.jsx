import React from 'react';
import Card from '../../../../components/common/Card';
import Button from '../../../../components/common/Button';
import FormInput from '../../../../components/common/FormInput';
import Select from '../../../../components/common/Select';
import { Trash2, Plus } from 'lucide-react';

export default function OccupancySection({ data = [], onChange }) {
  
  const handleAdd = () => {
    onChange([...data, { timePeriod: '', weekdayOccupancy: '', weekendOccupancy: '' }]);
  };

  const handleRemove = (index) => {
    onChange(data.filter((_, i) => i !== index));
  };

  const handleChange = (index, field, value) => {
    const newData = [...data];
    newData[index] = { ...newData[index], [field]: value };
    onChange(newData);
  };

  return (
    <Card padding="1.5rem">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <h3>Occupancy</h3>
        <Button variant="ghost" onClick={handleAdd} type="button">
          <Plus size={16} style={{ marginRight: '0.5rem' }} /> Add Period
        </Button>
      </div>

      {data.length === 0 ? (
        <div style={{ padding: '2rem', textAlign: 'center', border: '1px dashed var(--border-glass)', borderRadius: 'var(--radius-md)', color: 'var(--text-secondary)' }}>
          No entries yet. Click "Add Period" to start.
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {data.map((item, index) => (
            <div key={index} style={{ padding: '1rem', background: 'rgba(15, 23, 42, 0.4)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-glass)' }}>
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '0.5rem' }}>
                <button type="button" onClick={() => handleRemove(index)} style={{ background: 'none', border: 'none', color: 'var(--error)', cursor: 'pointer' }}>
                  <Trash2 size={16} />
                </button>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 500, fontSize: '0.875rem' }}>Time Period</label>
                  <Select 
                    value={item.timePeriod || ''} 
                    onChange={(e) => handleChange(index, 'timePeriod', e.target.value)}
                    options={[
                      { value: 'MORNING', label: 'Morning' },
                      { value: 'DAYTIME', label: 'Daytime' },
                      { value: 'EVENING', label: 'Evening' },
                      { value: 'NIGHT', label: 'Night' }
                    ]}
                  />
                </div>
                <FormInput
                  label="Weekday Occupancy (Count)"
                  type="text"
                  value={item.weekdayOccupancy || ''}
                  onChange={(e) => handleChange(index, 'weekdayOccupancy', e.target.value)}
                />
                <FormInput
                  label="Weekend Occupancy (Count)"
                  type="text"
                  value={item.weekendOccupancy || ''}
                  onChange={(e) => handleChange(index, 'weekendOccupancy', e.target.value)}
                />
              </div>
            </div>
          ))}
        </div>
      )}
    </Card>
  );
}
